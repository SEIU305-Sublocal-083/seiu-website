...
## 2026-05-18 - [Add `role="status"` to dynamically rendered empty states]
**Learning:** When using JavaScript to dynamically replace content with an empty state (e.g., "No results found"), screen readers may not announce the change, leaving visually impaired users unaware that the search or filter completed with zero results.
**Action:** Always add `role="status"` to the container element of dynamically rendered empty states so assistive technologies automatically announce the update.
