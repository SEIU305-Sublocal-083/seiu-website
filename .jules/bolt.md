...
## 2026-07-12 - [String Concatenation over Regex for Date Parsing]
**Learning:** [Using regex replacement like `dateString.replace(/-/g, '/')` to parse dates for local timezone formatting is significantly slower than appending a time string like `dateString + 'T00:00:00'` due to regular expression engine overhead.]
**Action:** [When coercing YYYY-MM-DD date strings into Date objects for local timezone formatting, use string concatenation instead of regex replacement to avoid overhead and improve performance.]
